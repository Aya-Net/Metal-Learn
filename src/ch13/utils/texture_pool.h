...
class TexturePool {
  ...
  public:
    unsigned int loadTexture(const std::string &imagePath, MTL::Device* device) {
        if (texture_map_.find(imagePath) != texture_map_.end()) {
            std::cout << "Texture already loaded: " << imagePath << std::endl;
            return texture_map_[imagePath];
        }
        int texWidth, texHeight, texChannels;
        stbi_uc* pixels = stbi_load(imagePath.c_str(), &texWidth, &texHeight, &texChannels, 0);
        if (!pixels) {
        ...
        texture_map_[imagePath] = textures_.size() - 1;
