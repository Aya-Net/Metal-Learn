...
MTL::RenderPipelineState *Shader::createPipeline(MTL::Device *device, MTL::VertexDescriptor* vertexDescriptor, MTL::RenderPipelineColorAttachmentDescriptor *colorAttachmentDescriptor) {
    if (!lib_) {
        std::cout << "Shader not compiled yet. Call Compile() first." << std::endl;
        return nullptr;
    }

    MTL::Function *vert_func_ = lib_->newFunction(vertex_name_);
    MTL::Function *frag_func_ = lib_->newFunction(fragment_name_);

    MTL::RenderPipelineDescriptor* pipelineDescriptor = MTL::RenderPipelineDescriptor::alloc()->init();
    pipelineDescriptor->setVertexFunction(vert_func_);
    pipelineDescriptor->setFragmentFunction(frag_func_);
    pipelineDescriptor->setVertexDescriptor(vertexDescriptor);
    if (colorAttachmentDescriptor) {
        pipelineDescriptor->colorAttachments()->object(0)->release();
        pipelineDescriptor->colorAttachments()->setObject(colorAttachmentDescriptor->copy(), 0);
    } else {
        // Default to a common format if none provided
        MTL::RenderPipelineColorAttachmentDescriptor* defaultColorAttachment = pipelineDescriptor->colorAttachments()->object(0);
        defaultColorAttachment->setPixelFormat(MTL::PixelFormat::PixelFormatBGRA8Unorm);
    }
