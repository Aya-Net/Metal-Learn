...
*/
    ...
    MTL::Buffer *screenBuffer = device->newBuffer(quadVertices, sizeof(transparentVertices), MTL::ResourceStorageModeShared);

    TexturePool texturePool;

    texturePool.loadTexture("../../assets/container.jpg", device, true);
    ...
    Shader screenShader("shaders/screen_shader.metal", "vertex_screen", "fragment_screen");
    shader.Compile(device);
    screenShader.Compile(device);
    ...
    MTL::RenderPipelineState *screenPipeline = screenShader.createPipeline(device, vertexDescriptor);
    ...
    MTL::TextureDescriptor *drawableDesc = MTL::TextureDescriptor::texture2DDescriptor(
    ...
    drawableDesc->setStorageMode(MTL::StorageModePrivate);
    MTL::Texture* drawableTexture = device->newTexture(drawableDesc);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        //NS::AutoreleasePool* pool = NS::AutoreleasePool::alloc()->init();
        {
            ...
            MTL::RenderCommandEncoder* renderCommandEncoder = cmd->renderCommandEncoder(renderPassDescriptor);
            {
                ...
                renderCommandEncoder->setFrontFacingWinding(MTL::Winding::WindingClockwise);
                renderCommandEncoder->setCullMode(MTL::CullMode::CullModeFront);
                ...
                renderCommandEncoder->setCullMode(MTL::CullMode::CullModeNone);
...
//                }
            ...
            MTL::RenderPassDescriptor *screenRenderPassDescriptor = MTL::RenderPassDescriptor::alloc()->init();
            MTL::RenderPassColorAttachmentDescriptor *screenColorAttachment = screenRenderPassDescriptor->colorAttachments()->object(0);
            screenColorAttachment->setTexture(surface->texture());
            screenColorAttachment->setLoadAction(MTL::LoadActionClear);
            screenColorAttachment->setStoreAction(MTL::StoreActionStore);
            MTL::RenderCommandEncoder* screenRenderCommandEncoder = cmd->renderCommandEncoder(screenRenderPassDescriptor);
            {
                screenRenderCommandEncoder->setRenderPipelineState(screenPipeline);
                screenRenderCommandEncoder->setVertexBuffer(screenBuffer, 0, 0);
                screenRenderCommandEncoder->setFragmentTexture(drawableTexture, 0);
                screenRenderCommandEncoder->setFragmentSamplerState(screenSamplerState, 0);
                screenRenderCommandEncoder->drawPrimitives(MTL::PrimitiveTypeTriangle, NS::UInteger(0), 6);
            }
            screenRenderCommandEncoder->endEncoding();
            screenRenderPassDescriptor->release();
            screenRenderCommandEncoder->release();
